...
```{r}
  ...
  project.dir <- "/Users/erinc/Desktop/DataMunging/Projects/ClassProject2"
  dataset.dir <- "/data"
...
```{r, echo=FALSE}
  ...
  # first, download the original file from:
  # https://data.cms.gov/Medicare/Inpatient-Prospective-Payment-System-IPPS-Provider/97k6-zzx3
  
  # read the file
  top100.dt <- fread(
                  file.path(project.dir, dataset.dir, "AmountChargedTop100Procedures.csv")
                )
  # get rid of spaces in names (we will improve them all later) - this is just to make the set workable.
...
```{r}
  ...
  names_factors = c("ProviderId", "ProviderName", "ProviderStreetAddress", "ProviderCity", "ProviderState", "HospitalReferralRegionDescription")
...
```{r}
  ...
  setnames(
    top100.dt, 
    names(top100.dt), 
    c("provider.id", "provider.name", "address", "city", "state", "zip", "region", "num.discharges", "covered.charges", "total.payments", "medicare.payments", "drg.code", "drg.description")
  )
...
```{r}
  ...
  write.csv(top100.dt, file = "Top100Procedures.csv", row.names = FALSE)
