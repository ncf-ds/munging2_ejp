...
```{r}
  ...
  msdrg.dt[,cc := !grepl('w/o M?CC',Title) & grepl('[^M]CC',Title) ]
  msdrg.dt[,mcc := !grepl('w/o M?CC',Title) & grepl('MCC',Title)]
  msdrg.dt[,Title := NULL]
  setnames(msdrg.dt,names(msdrg.dt),
           c('drg.code','drg.mdc','drg.type','drg.cc','drg.mcc'))
...
```{r}
  ...
  write.csv(top100.dt, file = "./data/Top100Procedures.csv", row.names = FALSE)
...
```{r}
  ...
  #this should be 474412 rows by 18 columns
  #dim(top100.dt)
